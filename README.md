...
| `RATE_LIMIT_UPLOAD_PER_MINUTE` | `0` | *Opt-in* uploads per app per 60s window. **`0` (default) = disabled**; edge limiting (Nginx `limit_req`/CDN) is the primary control, and enabling this writes to SQLite. |
| `RATE_LIMIT_FILES_PER_MINUTE` | `0` | *Opt-in* download/delete requests per app per 60s. `0` (default) = disabled for the same reason. |
